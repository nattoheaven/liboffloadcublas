...
#define XHEMM_SUB(X) X##hemm_
#define XHEMM(X) XHEMM_SUB(X)
#define CUBLASXHEMM_SUB(XX) cublas##XX##hemm
#define CUBLASXHEMM(XX) CUBLASXHEMM_SUB(XX)
#define CUBLASXGEMM_SUB(XX) cublas##XX##gemm
#define CUBLASXGEMM(XX) CUBLASXGEMM_SUB(XX)

void
XHEMM(X)(char *__restrict__ side_, char *__restrict__ uplo_, INTEGER *__restrict__ m_, INTEGER *__restrict__ n_, T *__restrict__ alpha_, T *__restrict__ a, INTEGER *__restrict__ lda_, T *__restrict__ b, INTEGER *__restrict__ ldb_, T *__restrict__ beta_, T *__restrict__ c, INTEGER *__restrict__ ldc_)
{
  INTEGER info;
  INTEGER m, n, nrowa;
  ...
  default:
    info = 1;
    xerbla_(MACRO2STR(XX) "HEMM ", &info);
    return;
  }
  switch (*uplo_) {
  case 'u':
  ...
  if (handle != 0 && d_a != 0 && d_b != 0 && d_c != 0) {
    ...
    if (f == 0) {
      f = get_symbol(MACRO2STR(X) "hemm_");
    }
