...
#define XSYRK_SUB(X) X##syrk_
#define XSYRK(X) XSYRK_SUB(X)
#define CUBLASXSYRK_SUB(XX) cublas##XX##syrk
#define CUBLASXSYRK(XX) CUBLASXSYRK_SUB(XX)
#define CUBLASXGEMM_SUB(XX) cublas##XX##gemm
#define CUBLASXGEMM(XX) CUBLASXGEMM_SUB(XX)

void
XSYRK(X)(char *__restrict__ uplo_, char *__restrict__ trans_, INTEGER *__restrict__ n_, INTEGER *__restrict__ k_, T *__restrict__ alpha_, T *__restrict__ a, INTEGER *__restrict__ lda_, T *__restrict__ beta_, T *__restrict__ c, INTEGER *__restrict__ ldc_)
{
  INTEGER info;
  INTEGER n, k, nrowa;
  ...
  default:
    info = 1;
    xerbla_(MACRO2STR(XX) "SYRK ", &info);
    return;
  }
  switch (*trans_) {
  case 'n':
  ...
  alpha = *alpha_;
  beta = *beta_;
  if (n == 0 || ((EQUAL(alpha, 0.0) || k == 0) && EQUAL(beta, 1.0))) {
    return;
  }
  if (EQUAL(alpha, 0.0)) {
    ptrdiff_t i, j;
  ...
  if (handle != 0 && d_al != 0 && d_ar != 0 && d_c != 0) {
    ...
    if (f == 0) {
      f = get_symbol(MACRO2STR(X) "syrk_");
    }
